...
export async function getExams(): Promise<ExamRow[]> {
  const { data, error } = await supabase
    .from('exams')
    .select('id, title, conducting_body, description, exam_type, official_website, registration_start, registration_end, exam_date, result_date, application_fee, selection_process, eligibility, course, branch, minimum_semester, maximum_age, minimum_percentage, average_salary, status, created_at, updated_at')
