...
type ToneKey = 'violet' | 'blue' | 'pink' | 'amber'
...
function OrbitCard({ icon: Icon, label, sub, style, floatClass, iconAnim, tone }: OrbitCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="text-left">
        <div className="text-sm font-semibold leading-tight text-brand-text dark:text-white">{label}</div>
        <div className="text-[11px] text-brand-muted dark:text-brand-dark-muted">{sub}</div>
      </div>
    </motion.div>
  )
}
